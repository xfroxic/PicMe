...
const Pin = ({ pin: { postedBy, image, _id, destination, save} }) => {
  ...
  const savePin = (id) => {
    if (!alreadySaved) {
      ...
      // use Sanity documentation for reference, can also hover cursor over code for built-in documentation
      client
        // patch post with an id
        .patch(id)
        // add an array to track saves
        .setIfMissing({ save: [] })
        // insert document
        .insert('after', 'save[-1]', [{
          _key: uuidv4(),
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex gap-2">
                {/* add capability to download images */}
                <a 
                href={`${image?.asset?.url}?dl=`}
                download
                onClick={(e) => e.stopPropagation()}  // this allows the download to happen without also clicking through to the pin link
